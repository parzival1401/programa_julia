...
function create_focus_controls(fig, row, col)
    ...
    return grid, buttons, slider
end

grid_xy, buttons_xy, slider_xy = create_focus_controls(fig, 3, 3)
grid_xz, buttons_xz, slider_xz = create_focus_controls(fig, 3, 5)
grid_yz, buttons_yz, slider_yz = create_focus_controls(fig, 3, 4)
...
function create_model_listener(buttons, z_slider, menu, model_epi, model_tirf, dim)
    lift(buttons[1].clicks, buttons[2].clicks, z_slider.sliders[1].value, menu.selection) do in, out, angle, mode
        ...
        effective_point = dim == :xy ? midle_point_z : (dim == :xz ? midle_point_y : midle_point_x)
        effective_point = clamp(effective_point + in - out, 1, dim == :xy ? size_z : (dim == :xz ? size_y : size_x))
        
        slice = if dim == :xy
            model[angle][midle_point_x-img_mid_x:midle_point_x+img_mid_x, midle_point_y-img_mid_y:midle_point_y+img_mid_y, effective_point]
        elseif dim == :xz
            model[angle][midle_point_x-img_mid_x:midle_point_x+img_mid_x, effective_point, :]
        else  # yz
            model[angle][effective_point, midle_point_y-img_mid_y:midle_point_y+img_mid_y, :]
        end
...
model_xy = create_model_listener(buttons_xy, z_slider, menu, model_epi, model_tirf, :xy)
model_yz = create_model_listener(buttons_xz, z_slider, menu, model_epi, model_tirf, :xz)
model_xz = create_model_listener(buttons_yz, z_slider, menu, model_epi, model_tirf, :yz)
